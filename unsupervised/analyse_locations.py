...
property size (bedrooms, bathrooms and garages) and its location
"""
...
def load_data():
	...
	# drop the type field, that's what we'll try to recreate
	property_data.drop(['Type', 'Latitude', 'Longitude'], axis = 1, inplace = True)
...
def remove_outliers(log_data):
	...
	for feature in log_data.keys():
		...
		# as these are categorial variables don't consider them outliers
		...
		# if there's no spread, ignore it
		if Q1 == Q3:
			continue
		print feature, ":", Q1, " <->", Q3
	...
	# show relationship between remoteness and price
	sns.jointplot(x=good_data['Price'], y=good_data['Remoteness'], kind="kde")
	plt.show()
...
def do_data_recovery(pca, centers, property_data):
	...
	# Determine the percentile for the centroid values to aid interpretation
	seg_ranks = pd.DataFrame(columns={})
	seg_ranks['Price'] = np.around([stats.percentileofscore(property_data['Price'], i) for i in true_centers['Price']], 0)
	seg_ranks['Remoteness'] = np.around([stats.percentileofscore(property_data['Remoteness'], i) for i in true_centers['Remoteness']], 0)

	print "\nPercentile ranks of each segment centroid:"
	display(seg_ranks)
